...
if ($vou_format == 1){
    if ($_GET['save'] == 'vou_adv') {
        ...
        $stmt = $mysqli->query("INSERT INTO emp_adv_given (emp_id, emp_name, vou_format, vou_date, jc_num, given_emp_id, given_amt, adv_details) 
            VALUES ('$emp_id', '$emp_name', '$vou_format', '$vou_date', '$jc_num', '$given_emp_id', '$given_amt', '$adv_details')") or die($mysqli);
        ...
        if (isset($_GET['edit']) == 'yes') {
            ...
            $stmt_emp_adv = "UPDATE emp_adv_given SET
                ...
                given_amt = '$given_amt', 
                adv_details = '$adv_details' WHERE s_no = '$serial_no'";
        ...
        header("location: voucher_index.php?serial_no=$serial_no&vou_format=$vou_format" . "&message=" . $message);
            ...
            $message = "Voucher Details:  Voucher Id:" . $serial_no . "details successfully Created...";
                ...
                for ($i = 0; $i < count($jc_nums); $i++) {
                    ...
                    // Insert the data into the database
                    $stmt = $mysqli->query("INSERT INTO emp_adv_given (emp_id, s_no, emp_name, vou_format, vou_date, jc_num, given_emp_id, given_amt, file_path, creation_date) 
                        VALUES ('$emp_id', '$serial_no', '$emp_name', '$vou_format', '$vou_date', '$jc_num', '$given_emp_id', '$given_amt', '$file_path', '$creation_date')") or die($mysqli);
