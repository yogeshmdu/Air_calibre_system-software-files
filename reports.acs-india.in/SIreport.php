...
<head>
    ...
    <title>JC-<?php echo ltrim($selectedReportId, 'REP_'); ?> SL REPORT</title>
...
<body>
    ...
    <?php
    // AHU Details 
    $query2 = ("SELECT * FROM reports_ahu_details WHERE report_id='$selectedReportId' AND sl_qty > 0 AND testing_date = '$testing_date'") or die($mysqli);
    ...
    if (mysqli_num_rows($result2) > 0) {
        while ($row2 = $result2->fetch_assoc()):
            ...
            $instrument = $row2['sl_instrument_used'];
            ...
            // Obtained results for sound level 
            $query4 = ("SELECT * FROM reports_sl_data WHERE ahu_seq_no ='$ahu_seq' ") or die($mysqli);
            ...
            for ($i = 1; $i <= $pr; $i++):
                ...
                $slquery4 = ("SELECT * FROM reports_sl_data WHERE ahu_seq_no ='$ahu_seq' limit $data_per,$perpage") or die($mysqli);
                $slresult4 = $mysqli->query($slquery4);
                $slrowcount = mysqli_num_rows($slresult4);
                ...
                <table id="velo">
                    ...
                    <tr>
                        ...
                        <td colspan="2"><span style="font-weight: bold;">Sound Level Test (dB)</span></td>
                        <td rowspan="2"><span style="font-weight: bold;">status</span></td>
                    </tr>
                    <tr>
                        <td><span style="font-weight: bold;">Acceptance Limit <sapn>
                        </td>
                        <td>
                            <sapn style="font-weight: bold;">Observed Limit</sapn>
                        </td>
                    </tr>
                    <tr>
                        <td rowspan="<?php echo $sl_quty; ?>">
                            <?php echo $num_of_ahu++; ?>
                        </td>
                        <td rowspan="<?php echo $sl_quty; ?>">
                            <?php echo $room_detai; ?>
                        </td>
                        <?php while ($sldata = $slresult4->fetch_assoc()): ?>
                            <td>
                                <?php echo $sldata['acceptance_limit']; ?>
                            </td>
                            <td>
                                <?php echo $sldata['observed_limit']; ?>
                            </td>
                            <td>
                                <?php echo $sldata['status']; ?>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                ...
                switch ($slrowcount) {
                    case 1:
                        echo '<div id="empty" style="height:300px; "></div>';
                        break;
                    case 2:
