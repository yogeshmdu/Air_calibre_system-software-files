...
<head>
    ...
    <title>JC-<?php echo ltrim($selectedReportId, 'REP_'); ?> FI REPORT</title>
...
<body>
    ...
    <?php
    // AHU Details 
    $query2 = ("SELECT * FROM reports_ahu_details WHERE report_id='$selectedReportId' AND filter_integrity_qty > 0  AND testing_date = '$testing_date'") or die($mysqli);
    ...
    if (mysqli_num_rows($result2) > 0) {
        while ($row2 = $result2->fetch_assoc()):
            ...
            $instrument = $row2['fi_instrument_used'];
            ...
            $fi_quty = $row2['filter_integrity_qty'];

            $tested_name = $row2['testing_engg'];
            // echo $tested_name;
            $fi_txtarea = $row2['fi_txtarea'];
            ...
            $query4 = ("SELECT * FROM reports_filter_integrity WHERE ahu_seq_no ='$ahu_seq'") or die($mysqli);
            ...
            for ($i = 1; $i <= $pr; $i++):
                ...
                $fiquery4 = ("SELECT * FROM reports_filter_integrity WHERE ahu_seq_no ='$ahu_seq' limit $data_per,$perpage") or die($mysqli);
                $firesult4 = $mysqli->query($fiquery4);
                $firow_count = mysqli_num_rows($firesult4);
                ?>
                ...
                <table id="customer">
                    <tr>
                        <th colspan="4">FILTER INTEGRITY&nbsp; TEST REPORT</th>
                    </tr>
                    
                    <tr>
                        <td class="cust_first" ><span style="font-weight: bold;"> Certificate No.</span> </td>
                        <td class="cust_sec" style="border-top: 1px dotted;">
                            <span style="font-weight: bold;">
                                <?php echo $job_card; ?> &#47; FI - <?php 
                                    if($print_data != 0) {
                        ...
                        <td class="cust_first" style="border-top: 1px dotted;"> Test Condition </td>
                        <td class="cust_sec" style="border-top: 1px dotted;">
                ...
                <table id="velo">
                    ...
                    // ============================== FILTER INTEGRITY TESTING VALUES ==============================
        
                    ?>
                    <tr>
                        <th colspan="9">OBTAINED &nbsp; TEST RESULTS</th>
                    </tr>
                    <tr>
                        ...
                        <td colspan="3"><span style="font-weight: bold;">Up stream<br> Concentration ( &#37; )</span></td>
                        <td colspan="2"><span style="font-weight: bold;">Down stream Penetration ( &#37; )</span>
                        </td>
                        <td rowspan="2"><span style="font-weight: bold;">status</span></td>
                    </tr>
                    <tr>
                        <td><span style="font-weight: bold;">&micro;g/L </span>
                        </td>
                        <td>
                            <span style="font-weight: bold;">Before<br>Scanning</span>
                        </td>
                        <td>
                            <span style="font-weight: bold;">After<br>Scanning</span>
                        </td>
                        <td><span style="font-weight: bold;">HEPA Filter<br>effective area</span></td>
                        <td><span style="font-weight: bold;">Fitment with<br>Frame Leakage</span></td>

                    </tr>
                    <tr>

                        <td rowspan="<?php echo $fi_quty; ?>">
                            <?php echo $num_of_ahu++; ?>
                        </td>
                        <td rowspan="<?php echo $fi_quty; ?>">
                            <?php echo $room_detai; ?>
                        </td>
                        <?php while ($fidata = $firesult4->fetch_assoc()): ?>
                            <td>
                                <?php echo $fidata['filter_name']; ?>
                            </td>
                            <td>
                                <?php echo $fidata['micro_gram']; ?>
                            </td>
                            <td>
                                <?php echo $fidata['before_upstream']; ?>&#37;
                            </td>
                            <td>
                                <?php echo $fidata['after_upstream']; ?>&#37;
                            </td>
                            <td>
                                <?php echo $hepa_fi = $fidata['hepa_filter_efficiency']; ?>&#37;
                            </td>
                            <td>
                                <?php echo $fidata['fitment_leakage']; ?>&#37;
                            </td>
                            <td>
                                <?php if (0.01 >= $hepa_fi) {
                                    echo $pas;
                ...
                <?php if ($fi_txtarea = true) { ?>
                    <table style="border-top:1px dotted; border-right: 1px dotted; border-left: 1px dotted; ">
                        <td>
                            <span style="font-weight: bold; ">&nbsp;INFERENCE : The above test results meets the acceptance
                                criteria.</span><br>
                            &nbsp;Test spot for Each Filter: Filter face and Fitment area fully scanned. The scanning of HEPA filter
                            area done at 1" away from the filter
                            Face moving the probe at a speed of 10ft/min with over lapping stroke &nbsp;and the Acceptance criteria for
                            PAO Downstream Penetration at
                            filter effective area is &#8804; 0.01%, for Filter Fitment with Frame Leakage should be NIL. <br>
                            <?php echo $row2['fi_txtarea']; ?>
                        </td>
                    </table>
                <?php } else { ?>
                    <table style="border-top:1px dotted; border-right: 1px dotted; border-left: 1px dotted;">
                        <td>
                            ...
                            Face moving the probe at a speed of 10ft/min with over lapping stroke &nbsp;and the Acceptance criteria for
                            PAO Downstream Penetration at
                            filter effective area is &#8804; 0.01%, for Filter Fitment with Frame Leakage should be NIL.
                        </td>
                    </table>
                <?php } ?>
